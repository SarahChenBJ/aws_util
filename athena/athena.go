...
func (c *AthenaEngine) ExecuteQuery(qi *RequestParam) (queryID string, err error) {
	log.Printf("[Executing Athena Query] %s", qi)
	...
	if err != nil {
		log.Printf("Athena Query Error: %s", err.Error())
		return "", err
	}
	log.Printf("[Finished Athena Query] QueryID: %s", aws.StringValue(output.QueryExecutionId))
