...
//https://www.jianshu.com/p/20b533c5c3f9
// 1. go/token & go/scanner: help to translate the file into tokens, e.g. : package, import, const, struct, function ...
// 2. go/parser: convert the tokens into ast(Abstract Syntax Tree):
//				*ast.TypeSpec
//					-> *ast.StructType
//						-> *ast.FieldList
//							-> *ast.Field
...
//								-> *ast.BasicLit
//								-> *ast.Ient
// 3. Pinpoint the struct: get the start and end of the struct codes
// 4. Iterate the ast.Node to store the struct as structtag
// 5. Add tags for each field
func ParseDDBStruct(structName, file string) error {
	//outfile := strings.Replace(file, ".go", "_ddb.go", -1)

	cfg := &config{
		//The input file where the orignal struct stays
		file: file,

		//The name of the struct which is waiting to be tagged
		structName: structName,

		//The output format: source or json
		output: "source",

		//true: the new struct will be written to the output file, or else it'll be output on the screen
		write: true,

		//The add tags: it'll add json and ddb as the tag
		add: []string{"json", "ddb"},

		//All tag name will be formed as snakecase of the field name
		transform: "snakecase",

		//True: means only the public field will be tagged
