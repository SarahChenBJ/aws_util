...
func funcs(iface string, srcDir string) ([]Func, error) {
	...
	if err != nil {
		fmt.Println("findInterface error")
		return nil, err
	}
...
func GenStubs(iface, outdir, structName, outfile string) error {
	fns, err := funcs(iface, outdir)
	if err != nil {
		return err
	}

	src := genStubs(structName, fns)
	return appendToFile(src, outfile)
}

func appendToFile(src []byte, outfile string) error {
	fd, e := os.OpenFile(outfile, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0644)
	if e != nil {
		return e
	}
	_, e = fd.Write(src)
